...
const App: React.FC = () => {
  ...
  const loadAlarms = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:5001/api/alarms');
  ...
  const loadCubeStatus = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:5001/api/cube/status');
      const data = await response.json();
  ...
  const handleSaveAlarm = async (alarmData: Omit<Alarm, 'id'>) => {
    try {
      const url = editingAlarm 
        ? `http://127.0.0.1:5001/api/alarms/${editingAlarm.id}`
        : 'http://127.0.0.1:5001/api/alarms';
  ...
  const handleDeleteAlarm = async (id: string) => {
    try {
      const response = await fetch(`http://127.0.0.1:5001/api/alarms/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleToggleAlarm = async (id: string, enabled: boolean) => {
    try {
      const response = await fetch(`http://127.0.0.1:5001/api/alarms/${id}/toggle`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleStopAlarm = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5001/api/alarms/stop', {
        method: 'POST',
      });
  ...
  const handleResetCubeState = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5001/api/cube/reset', {
        method: 'POST',
      });
  ...
  const handleConnectCube = async () => {
    try {
      await fetch('http://127.0.0.1:5001/api/cube/connect', { method: 'POST' });
    } catch (error) {
  ...
  const handleDisconnectCube = async () => {
    try {
      await fetch('http://127.0.0.1:5001/api/cube/disconnect', { method: 'POST' });
    } catch (error) {
